...
\begin{figure}
    \lstinputlisting[language=llvm,style=nasm]{example.ll}
...
Based on the method for generating MBA identities proposed by Zhou et al. \cite{mba_zhou}, Eyrolles has generated a list of all rewrite rules composed of three boolean expressions \cite{eyrollesMBAobf}. We have decided to implement those rules in the obfuscator, while also providing the user with an option to apply the rules in multiple iterations with randomized selection of a rewrite rule to increase code complexity and diversity. All of the implemented rewrite rules can be found in Appendix \ref{app:a}.
...
\begin{itemize}
    ...
    \item $g = f^{-1}$
    \item $E$ an MBA expression non-trivially equal to zero, for example\footnote{Obtained from the 2nd rewrite rule in Appendix \ref{app:a}.} $E = x + y - (x \vee y)-(\neg x \vee y)+(\neg x)$
...
Strings, i.e. sequences of characters, are present in most of the existing programs. They are used to improve accesibility for the user interacting with the program, or to produce meaningful logging and error messages for the developers and so on. For the reverse engineer, strings can present a valuable source of information about the program. %TODO elaborate, mention strings command

LLVM provides a convenient way to obfuscate all strings used in the program. During the obfuscation, the strings can be encoded using an arbitrary function which transforms them. Then, a decoding function is injected into the LLVM IR module. During runtime, the decoding function is used to obtain the original string, when needed. All the other strings remain encoded when not being used. This approach provides resilience against static analysis, while also introducing more complexity when performing dynamic analysis. 

The encoding and decoding functions can be different for each obfuscated program, which gives us a choice to either reduce the computational overhead by using an efficient and simple encoding and decoding function, or to strengthen the obfuscation by using more complex cryptographic functions. When a keyed function is used, the key needs to be included in the obfuscated program. Other obfuscation methods, such as the one mentioned in Section \ref{obf_const}, can be used to hide the key.

\subsection{Encoding and decoding functions}

\section{Bogus Control Flow}

\section{Control Flow Flattening}
...
\chapter{MBA Rewrite Rules} \label{app:a}
...
&x+y \rightarrow-(x \oplus y)+2 y+2 \times(x \wedge \neg y) \\
&x+y \rightarrow 2 y-(\neg x \wedge y)+(x \wedge \neg y) \\
&x+y \rightarrow 2 y-(\neg x)+(\neg y) \\
...
&x \oplus y \rightarrow 2 \times(-1)-(\neg x \vee y)-(x \vee \neg y) \\
&x \oplus y \rightarrow-y+2 \times(\neg x \wedge y)+x \\
&x \oplus y \rightarrow-(\neg x \vee y)+2 \times(\neg x \wedge y)+(x \vee \neg y) \\
&x \oplus y \rightarrow y+x-2 \times(x \wedge y) \\
&x \oplus y \rightarrow(\neg x \vee y)+(x \vee \neg y)-2 \times(\neg(x \oplus y)) \\
&x \oplus y \rightarrow y+(x \wedge \neg y)-(x \wedge y) \\
&x \oplus y \rightarrow y+(\neg y)-(\neg(x \oplus y)) \\
&x \oplus y \rightarrow(\neg x \vee y)+(x \wedge \neg y)-(\neg(x \oplus y))
\end{aligned}
$$
\section{AND}
...
&x \wedge y \rightarrow 1+y+(x \vee \neg y) \\
&x \wedge y \rightarrow(x \vee y)-(\neg x \wedge y)-(x \wedge \neg y) \\
&x \wedge y \rightarrow(-1)-(\neg x \wedge y)-(\neg y) \\
